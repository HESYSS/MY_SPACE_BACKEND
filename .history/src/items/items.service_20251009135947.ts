...
export class ItemsService {
  ...
  async findAll(filters: any) {
    ...
    if (filters.search) {
      ...
      itemConditions.push({ AND: searchClauses }); // Оборачиваем searchClauses в OR, чтобы поиск был по ЛЮБОМУ из полей
    }
  ...
  async getCoordinates(filters: any) {
    ...
    if (filters.search) {
      ...
      itemConditions.push({ AND: searchClauses }); // Оборачиваем searchClauses в OR
    }
