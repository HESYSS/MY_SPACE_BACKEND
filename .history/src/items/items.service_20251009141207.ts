...
export class ItemsService {
  ...
  async findAll(filters: any) {
    ...
    let orderBy: any = { updatedAt: "desc" }; // дефолт
    ...
    // если задан поиск — добавляем приоритет
    if (filters.search) {
      const searchTerm = filters.search.trim();
      orderBy = [
        // сначала те, где title/description содержит поисковый запрос
        Prisma.sql`CASE
      WHEN "Item"."title" ILIKE ${"%" + searchTerm + "%"}
        OR "Item"."description" ILIKE ${"%" + searchTerm + "%"}
        OR "Item"."titleEn" ILIKE ${"%" + searchTerm + "%"}
        OR "Item"."descriptionEn" ILIKE ${"%" + searchTerm + "%"}
      THEN 0
      ELSE 1
    END ASC`,
        // затем уже обычная сортировка (по дате или цене)
        orderBy,
      ];
    }
