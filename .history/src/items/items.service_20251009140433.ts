...
export class ItemsService {
  ...
  async findAll(filters: any) {
    ...
    if (filters.search) {
      const searchWhere = structuredClone(where);
      // В searchWhere оставляем только условия поиска
      searchWhere.AND = where.AND.filter((cond: any) => cond.OR);

      searchItems = await this.prisma.item.findMany({
        where: searchWhere,
        skip,
        take: limit,
        orderBy: { updatedAt: "desc" },
      ...
      // Убираем из фильтра поисковое условие, чтобы не дублировать
      where.AND = where.AND.filter((cond: any) => !cond.OR);
    }

    otherItems = await this.prisma.item.findMany({
      where,
      skip: filters.search ? 0 : skip, // чтобы не пропустить первые
      take: limit,
      orderBy,
      include: {
        location: true,
    ...
    const items = [...searchItems, ...otherItems].filter(
    ...
    return {
      items: items.map((item) => {
        ...
        const rooms =
          item.characteristics.find((c: any) => c.key === "room_count")
            ?.value ?? null;
        const area =
          item.characteristics.find((c: any) => c.key === "area_total")
            ?.value ?? null;
        ...
        return {
          ...
          metros: item.metros.map((m: any) => m.name),
