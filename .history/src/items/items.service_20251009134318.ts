...
export class ItemsService {
  ...
  async findAll(filters: any) {
    // === Используем 'q' как search, если передан ===
    ...
    if (filters.search) {
      ...
      itemConditions.push({ OR: searchClauses });
    }
      ...
      if (filters.district)
        locationConditions.push(
          ...String(filters.district)
            .split(",")
            .map((d) => ({ district: { in: [d], mode: "insensitive" } }))
        );
      itemConditions.push({ location: { is: { OR: locationConditions } } });
    }

    if (itemConditions.length > 0) andConditions.push({ OR: itemConditions });
    ...
    if (filters["prices.value_min"] || filters["prices.value_max"]) {
      ...
      andConditions.push({ prices: { is: { priceUsd: priceWhere } } });
    }
    ...
    if (
      ...
      const cond: any = { key: "floor", valueNumeric: {} };
    ...
    if (
      ...
      const cond: any = { key: "area_total", valueNumeric: {} };
    ...
    if (filters["characteristics.room_count"]) {
      ...
      charConditions.push({ key: "room_count", value: { in: values } });
    }
    if (charConditions.length > 0)
      andConditions.push(
        ...charConditions.map((c) => ({ characteristics: { some: c } }))
      );
    ...
    // --- Приоритет поиска (search сначала) ---
      ...
      prioritizedItems = [
        ...items.filter(
          (item) =>
            ...
            item.newbuildingName?.toLowerCase().includes(searchTerm) ||
    ...
    // --- Pagination после приоритета ---
    const paginatedItems = prioritizedItems.slice(skip, skip + limit);
    ...
    return {
      ...
      total: prioritizedItems.length, // возвращаем полное количество с учетом search
    };
  }
