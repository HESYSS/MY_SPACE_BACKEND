...
export class ItemsService {
  ...
  async findOne(id: string, lang: string) {
    ...
    return this.mapItemWithLang(item, lang);
  }

  private mapItemWithLang(item: any, lang: string) {
    // --- характеристики ---
    const characteristics: Record<string, any> = {};
    for (const c of item.characteristics) {
      const label = lang === "ua" ? c.key : c.keyEn;
      const value = lang === "ua" ? c.value : c.valueEn;

      if (!label) continue;
      if (value === null || value === undefined || value === "") continue;

      // добавляем м² к полям площади
      const areaKeys = [
        "area_total",
        "area_kitchen",
        "area_living",
      ...
      if (areaKeys.includes(c.key || "")) {
        characteristics[label] = `${value} ${lang === "ua" ? "м²" : "m²"}`;
      } else {
    ...
    const location =
      lang === "ua"
        ...
        : {
            country: item.location.countryEn,
            region: item.location.regionEn,
            city: item.location.cityEn,
            borough: item.location.boroughEn,
            district: item.location.districtEn,
            street: item.location.streetEn,
            streetType: item.location.streetTypeEn,
            lat: item.location.lat,
            lng: item.location.lng,
          };
