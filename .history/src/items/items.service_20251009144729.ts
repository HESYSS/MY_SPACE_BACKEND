...
export class ItemsService {
  ...
  async findAll(filters: any) {
    ...
    if (filters.search) {
      // Берем максимум 500, чтобы не тянуть весь массив
      ...
      const scoreItem = (item: any): number => {
        ...
        for (const field of fields) {
          ...
          else if (text.includes(searchTerm)) score += 5;
        }
        return score;
      };

      // сортировка по релевантности
      const sorted = allItems.sort((a, b) => scoreItem(b) - scoreItem(a));

      // пагинация вручную
      items = sorted.slice(skip, skip + limit);
    } else {
      // стандартная пагинация через Prisma
