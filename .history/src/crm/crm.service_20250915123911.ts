...
export class CrmService {
  ...
  async syncData(): Promise<void> {
    ...
    for (const raw of items) {
      ...
      console.log(`Обработка CRM ID=${dto.id}...`, dto.title);
      // --- Переводы ---
      ...
      await this.prisma.item.upsert({
        ...
        update: {
          status: dto.status,
          title: dto.title,
          titleEn: titleEn, // 👈 сохраняем перевод
          description: dto.description,
          descriptionEn: descriptionEn, // 👈 сохраняем перевод
          deal: dto.deal,
          type: dto.type,
          ...
          metros: {
            ...
            create: metrosWithTranslations.map((m) => ({
              name: m.name,
              nameEn: m.name_en, // 👈 поле перевода
              distance: m.distance,
            })),
          },
          ...
          characteristics: {
            ...
            create: characteristicsWithTranslations.map((c) => ({
              key: c.key,
              keyEn: c.key_en, // 👈 перевод ключа
              value: c.value,
              valueEn: c.value_en, // 👈 перевод значения
