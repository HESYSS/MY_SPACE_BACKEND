    ...
    for (const dto of this.dtoContainer) {
      ...
      const metrosWithTranslations = await Promise.all(
        (dto.location.metros || []).map(async (m) => ({
          ...m,
          nameEn: await this.translateService.translateText(m.name, "en"),
        }))
      );
      ...
      const characteristicsWithTranslations = await Promise.all(
        [
          ...
          ...c,
          keyEn: await this.translateService.translateText(c.key, "en"),
          valueEn: await this.translateService.translateText(c.value, "en"),
        }))
      );
      console.log(
        "characteristicsWithTranslations",
        characteristicsWithTranslations
      );
      // Сохраняем только переводы в БД
